...
func main() {
	...
	result, err := simpleMsg.Format(map[string]any{
		"name": "Alice",
	})
	if err != nil {
		log.Fatal(err)
	...
	result, err = multiMsg.Format(map[string]any{
		"firstName":    "John",
		"lastName":     "Doe",
		"messageCount": 3,
	})
	...
	for _, count := range counts {
		result, err := pluralMsg.Format(map[string]any{
			"count": count,
		})
		if err != nil {
			log.Fatal(err)
	...
	for _, gender := range genders {
		result, err := selectMsg.Format(map[string]any{
			"gender": gender,
		})
		if err != nil {
			log.Fatal(err)
